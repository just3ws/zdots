...
Scope note: bear3 is a Rails app on another machine and is **out of scope for execution** here (no bear3 content in prompt context). This strategy is a grounded projection from (a) graphify's validated Ruby AST behavior on `lib/zdots/` and (b) graphify's declared grammar/feature set. Run the benchmarks locally on bear3 to confirm.
...
## Recommended bear3 workflow (to run there, local-only)
...
graphify benchmark                             # confirm token reduction at bear3 scale
```
