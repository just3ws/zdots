...
Scope note: work3 is a Rails app on another machine and is **out of scope for execution** here (no work3 content in prompt context). This strategy is a grounded projection from (a) graphify's validated Ruby AST behavior on `lib/zdots/` and (b) graphify's declared grammar/feature set. Run the benchmarks locally on work3 to confirm.
...
## Recommended work3 workflow (to run there, local-only)
...
graphify benchmark                             # confirm token reduction at work3 scale
```
