...
2. For (b)/(c), sketch the commoditization surface: a mountable Rails engine / gem / generator vs. a set of zdots CLIs the app shells out to. Prefer the platform-service seam (per the [redacted]-tenant service-surface work, Z-184/Z-185) over per-app reimplementation.
