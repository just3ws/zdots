...
- LIVE nginx serves .local for all vhosts (llama/embed/o2/zdots/my/emrbear).
...
- CAUTION: zdots.localhost / my.localhost currently return 200 here, but that is the false-green decision-011 documents — no live .localhost server_name, so they fall through to emrbear.conf's `listen 80 default_server` (EMR-Bear app), NOT the real backends. On this machine .local is what actually reaches zdots/context-engine.

Goal: bring this machine to .localhost in STAGES — certs first (non-switching, safe anytime), vhost switch later when ready; emrbear.local->emrbear.localhost handled separately as EMR-Bear work wraps.
...
- [x] #3 emrbear.local -> emrbear.localhost migrated and the nginx default_server decision recorded
...
<!-- SECTION:PLAN:BEGIN -->
STAGE 1 — CERTS NOW (non-switching, safe): regenerate the shared cert to cover the UNION of current .local + incoming .localhost names. Extra SANs are harmless while vhosts stay .local. Pre-include emrbear.localhost (forward-looking).
  mkcert -cert-file "$(brew --prefix)/etc/nginx/certs/zdots-local.pem" \
         -key-file  "$(brew --prefix)/etc/nginx/certs/zdots-local-key.pem" \
    llama.localhost embed.localhost o2.localhost zdots.localhost gemstash.localhost my.localhost dev.my.localhost emrbear.localhost \
    llama.local embed.local o2.local zdots.local gemstash.local my.local dev.my.local emrbear.local grafana.local \
    localhost 127.0.0.1
...
STAGE 3 — EMRBEAR (pulled into the cutover 2026-07-11 by operator instruction): emrbear
dashboard repo (~/github.com/emrbear/dashboard) fully flipped — ops/nginx/emrbear.conf
server_names -> emrbear.localhost, dev config.hosts entry dropped (Rails dev default
already allows .localhost), links.yml/layout hostname-classifier/comments updated.
Deploy = cp ops/nginx/emrbear.conf to the live servers dir before the reload.
...
200'd from EMR-Bear with a _dashboard_session cookie) — becomes the tracked
...
VERIFY each switch (redirect-blind + IDENTITY — the decision-011 false-green lesson):
  for h in llama embed o2 zdots gemstash my emrbear; do printf '%s.localhost: ' "$h"; curl -sk "https://$h.localhost" -o /dev/null -w '%{http_code}\n' --max-redirs 0; done
  Then confirm the zdots names carry NO _dashboard_session (EMR-Bear) cookie — a bare 200 is not proof; it was 200 before, for the wrong reason.
<!-- SECTION:PLAN:END -->
...
EVIDENCE:
- AC#1 cert: 19-SAN union minted (mkcert output: 8x .localhost incl gemstash+emrbear, 8x legacy .local, grafana.local, localhost, 127.0.0.1), expires 2028-10-11. Backups .bak-20260711-114216.
...
- AC#3 emrbear: emrbear.localhost 200 + _dashboard_session (correct backend). Dashboard repo commit 68a5eb7 (conf, config.hosts dropped — Rails dev default allows .localhost, link classifier, links.yml). default_server decision: stock nginx.conf keeps :80 default (welcome page, nginx-ctl status probes it); tracked 00-default.conf takes :443 default → 404 + X-Zdots-Default header. Verified: nonesuch.localhost AND retired zdots.local both 404 + X-Zdots-Default. False-green class dead (pre-switch audit: ALL .localhost names 200'd from EMR-Bear).
...
- Commits: zdots 8af8a59 + 1347820 + ef3a348 (+ backlog close-out), ~/my be03259, emrbear dashboard 68a5eb7.
