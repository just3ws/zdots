...
command-qc criterion: non-interactive safety — every bin command must (1) answer --help without executing its action (zdots-man-gen fork-bombed on one that didn't, [redacted] hit the same class 2026-07-10: 'work-daily-refresh-run --help launched the whole daily chain') and (2) never block on a TTY prompt when stdin is not a terminal (capture --yes class). Wire both into command-qc + a contract test.
