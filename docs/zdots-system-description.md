...
- **Observable:** `otelcol-contrib` runs as a working OTLP receiver and every shell command is logged (with PHI scrubbing) and every AI invocation is metered — but zdots' own Platform Services (llama-server, otel-collector itself, zdots-worker, etc.) do not currently emit OTLP spans/logs themselves; they're health-probed directly (curl/pg_isready/launchctl), not OTel-instrumented. Confirmed 2026-08-03: `SELECT DISTINCT service_name FROM "default"` over 30 days returns only external apps that push to the collector (e.g. the operator's day-job app), never a zdots service name. Self-instrumenting zdots' own services is open work, not yet done.
