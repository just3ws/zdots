...
@test "env.sh: Sets XDG base directories" {
  ...
  run bash -c '. env.sh && echo $XDG_CONFIG_HOME'
  [ "$status" -eq 0 ]
  [ "$output" == "$HOME/.config" ]
}

@test "env.sh: Sets ZDOTDIR if not provided" {
  run bash -c 'unset ZDOTDIR; . env.sh && echo $ZDOTDIR'
  [ "$status" -eq 0 ]
...
@test "env.sh: Correctly handles ZDOTS_ENV_PROFILE=ci-act" {
  ...
  run bash -c '. env.sh && echo $HOMEBREW_PREFIX'
  [ "$status" -eq 0 ]
  [ "$output" == "" ]
}

@test "env.sh: Generates a 32-hex W3C Trace ID" {
  run bash -c '. env.sh && echo $ZDOTS_TRACE_ID'
  [ "$status" -eq 0 ]
  # Regex for 32 hex chars
  [[ "$output" =~ ^[0-9a-f]{32}$ ]]
}

@test "env.sh: Generates a 16-hex W3C Span ID" {
  run bash -c '. env.sh && echo $ZDOTS_SPAN_ID'
  [ "$status" -eq 0 ]
  # Regex for 16 hex chars
  [[ "$output" =~ ^[0-9a-f]{16}$ ]]
}
