...
@test "guard allows git commit on a work machine (fix-forward)" {
...
@test "guard allows git push on a home machine" {
  run bash -c "ZDOTS_CONTEXT=home ZDOTDIR='$REPO_ROOT' '$GUARD' <<< '{\"tool_input\":{\"command\":\"git push origin main\"}}'"
  [ "$status" -eq 0 ]
}
